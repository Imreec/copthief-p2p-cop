# Project Conventions — `copthief-p2p-cop` (final project)
...
> ⚑ **This repo:** the **cop agent** (lead repo — `copthief_core/` is developed here; role
> package `copthief_police`, wire role `"police"`). Sibling:
...
| 16 | **Email interlock** [FP] (ADR-0008) | Reporting is **automatic** (App E rule 32; rule 35 zeroes BOTH teams on a missing report). Resting state = `enabled=false` with **no recipient**; **no email is ever sent to an address Imree has not configured for that run** — authorization is the recipient, set before the match, never inside it. Runaway protection = the gatekeeper (rule 28) |
...
deviation → code disqualification; the D1=A compose amendment is **reverted by ADR-0008**, which
dropped the draft posture so the mandated scope holds literally; sending identity = the
