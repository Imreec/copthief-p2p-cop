# COST.md — computational-fairness ledger (cop agent)
...
> assertion. Sibling: [copthief-p2p-thief](https://github.com/Imreec/copthief-p2p-thief).
...
1. **The event log.** Every game's JSONL records each decision; every committed evidence
   log reports `0 tokens` (M2 Stage A g1–g4, the M3 scent friendly, the M5 friendly g1–g3
   — see `docs/evidence/`).
2. **The sealed per-step counts (M6-3) — cryptographically auditable.** Since M6-3 every
   ...
   matrix covers the token fields). Permanent CI pin:
...
- **Cloudflare named tunnel** (`copthief` on `imreeyal.com`): free plan; the domain was
  already owned. No per-request cost.
...
logged truthfully in `docs/PROMPTS.md`. Development tooling consumes tokens on the
developers' side; **none of it touches match-time execution**, which is what the league's
