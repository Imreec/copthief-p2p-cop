...
- ◐ **M7-1** Sparring host — safety half BUILT 2026-07-22 (`shared/sparring` guard: no tuned weights, no mail; `scripts/make_sparring_config.py` validates its own output). **The deployment itself (OI-4) + the 24h reachability DoD never happened; the league floor was met without it. Disposition: decide at M8 — deploy or close as not-pursued (KNOWN_LIMITATIONS candidate).**
- ☐ **M7-2** Kit additions (wire-contract page, JSON-schema + audit fixtures, netcheck/checklist, `_g<NN>` note, consensus-signature vector w/ Alon credit, scent-model vectors, lock-schema pin, wire-shape registrations) — **status unverified from this repo**; several items are believed landed by kit-side sessions. Verify against the kit repo before ticking or dropping.
...
- ◐ **M7-5** Counted series vs distinct teams — **two played, App F pass floor met** (anrbj666 M7-41; uoh-sqak M7-52, won +10 diversity). Third candidate best2934 in negotiation — **blocked on a mutual code-level handshake gap: their negotiate carries no kit-CORE terms/nonce/signature, so each side's gate refuses the other** (both sides informed 2026-08-08; no T until that closes).
