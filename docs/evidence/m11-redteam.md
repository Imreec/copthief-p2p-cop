...
|---|--------|-------------------|-------------|
| T1 | **Cage-building cop** — invest sub-threshold walls at tempo, close a k-wall pocket. The forecast (`wall_forecast.py`) looks exactly ONE wall ahead; `region_cap=24` saturates on a 49-cell board so a forming cage is invisible until it is nearly shut; `room_first` caps the ruling flight term at `flight_floor=3` so the evader never relocates while the cop builds. | **Proven: our own police-m10 converts doctrine-m10 32/32** (m9-study-arena). 14 walls = 14 free tempi and nothing in the score tuple prices them. | **FIXED (M11-1)**: k-wall forecast + wall-tempo flight lift + uncapped-region cage term, measured below. |
